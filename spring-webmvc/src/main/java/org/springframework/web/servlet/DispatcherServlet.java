...
/*
 * springMVC 核心前端控制器 单例 基于servlet 遵循servlet生命周期
 * springboot中：第一次请求被初始化，执行父类init方法初始化相关组件与容器
 * 后执行doDispatch处理本次请求
 *
 * */
public class DispatcherServlet extends FrameworkServlet {
    ...
    static {
        ...
        try {
            //DispatcherServlet.class的相对路径获取DEFAULT_STRATEGIES_PATH资源，并且注入启动参数
            ClassPathResource resource = new ClassPathResource(DEFAULT_STRATEGIES_PATH, DispatcherServlet.class);
            defaultStrategies = PropertiesLoaderUtils.loadProperties(resource);
            /* 文件中默认配置了HandlerAdapter实现类全限定名：1：HttpRequestHandlerAdapter 2：SimpleControllerHandlerAdapter 3：RequestMappingHandlerAdapter
             * HandlerMapping实现类全限定名：BeanNameUrlHandlerMapping RequestMappingHandlerMapping
             * 为之后反射初始化
             * */
